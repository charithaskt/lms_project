...
(function(BiblioGridActions) {
    ...
    BiblioGridActions.callback_save_inline = function(viewModel) {
        ...
        this.updateDependentGrid('#hold_grid');
    };
    ...
    BiblioGridActions.callback_add_hold = function(viewModel) {
        this.callback_create_form(viewModel);
    };
    ...
    BiblioGridActions.callback_save_hold = function(viewModel) {
        var holdGridView = viewModel.hold_grid_view;
        delete viewModel.hold_grid_view;
        this.grid.updatePage(viewModel);
        // Get client-side class of HoldGrid component by id (instance of App.ko.Grid or derived class).
        var holdGrid = $('#hold_grid').component();
        if (holdGrid !== null) {
            // Update rows of BiblioGrid component (instance of App.ko.Grid or derived class).
            holdGrid.updatePage(holdGridView);
            // Highlight item tab so the user will know it has updated list page.
            App.TabPane().highlight('#hold_tab');
            // Switch to item grid tab to show item changes.
            // window.location.hash = '#hold_tab';
        }
    };
