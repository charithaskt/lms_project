...
from intranet.models import Biblio, LocalCoverImages
def validate_request_data_localcoverimages(fn):
    def decorated(*args, **kwargs):
        # args[0] == GenericView Object
        data = args[0].request.data
        imageurl = data.get("imageurl")
        biblio_id = data.get("biblionumber")
        mime = magic.Magic(mime=True)
        file_mime_type = check_in_memory_mime(args[0].request.FILES['imageurl'])
        print("file_mime_type = ",file_mime_type)
        if file_mime_type not in valid_mime_types: 
        ...
        try:
            if LocalCoverImages.objects.get(biblionumber=biblio_id): 
                return Response(
                    data={
                        "message": "A cover image for the book with that id: %d already exists" %  biblio_id
                    },
                    status=status.HTTP_400_BAD_REQUEST
                )
            if not (imageurl or biblio_id)  :
                return Response(
                    data={
                        "message":"Both Biblionumber & Image Url are required",
                    },
                    status=status.HTTP_400_BAD_REQUEST
                )
