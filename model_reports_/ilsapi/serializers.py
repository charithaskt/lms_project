...
from intranet.models import Authors, Suggestion, Publisher, Biblio, Genre, EntryExitLogs
...
from intranet.models import LocalCoverImages, Biblio
class LocalCoverImagesSerializer(serializers.ModelSerializer):

    class Meta:
        model = LocalCoverImages
        fields = ("id","biblionumber", "imageurl")

    def update(self, instance, validated_data):
        instance.imageurl = validated_data.get("imageurl", instance.imageurl)
        biblio_ = validated_data.get("biblionumber")
        if biblio_ and not instance.biblionumber:
           biblio  = Biblio.objects.get(pk=biblio_)
           if biblio:
              instance.biblionumber = biblio
        instance.save()
        return instance

class LocalCoverImagesShowSerializer(serializers.ModelSerializer):
    biblio =  BibliosSerializer(read_only=True)
    class Meta:
        model = LocalCoverImages
        fields = ('id','biblio','imageurl')

class EntryExitLogsSerializer(serializers.ModelSerializer):
    class Meta:
        model = EntryExitLogs
        fields = ("id","borrower","cardnumber","timeofentry","timeofexit")
