...
from .serializers import GenreSerializer, BibliosShowSerializer, ProfileSerializer
...
from .decorators import validate_request_data_localcoverimages
from intranet.models import Borrowers, PatronPhotos, LocalCoverImages
from .serializers import BorrowersSerializer, PatronPhotosSerializer, PatronPhotosShowSerializer, LocalCoverImagesSerializer, LocalCoverImagesShowSerializer
import re
...
class ListCreateLocalCoverImagesView(generics.ListCreateAPIView):
    """
    GET localcoverimages/
    POST localcoverimages/
    """
    queryset = LocalCoverImages.objects.all()
    serializer_class = LocalCoverImagesSerializer
    #parser_classes = (JSONParser,)
    parser_class = (ImageUploadParser,)
    permission_classes = (permissions.IsAdminUser,)
    def get(self, request, *args, **kwargs):
        images_ = LocalCoverImages.objects.all()
        if not images_ is None:
           serializer = LocalCoverImagesShowSerializer(images_, many=True)
           return Response(
                  data=serializer.data,
           )
        else:
            return Response(
                data={
                    "message": "No cover image exist in the database"
                },
                status=status.HTTP_404_NOT_FOUND
            )
     
    @validate_request_data_localcoverimages
    def post(self, request, *args, **kwargs):
        data = request.data
        serializer = LocalCoverImagesSerializer(data=data)
        if serializer.is_valid(): 
           serializer.save()
           return Response(
                  data=serializer.data,
...
class LocalCoverImageDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    ...
    def get(self, request, *args, **kwargs):
        try:
            a_localcoverimage = self.queryset.get(pk=kwargs["pk"])
            return Response(LocalCoverImagesShowSerializer(a_localcoverimage).data)
        except LocalCoverImages.DoesNotExist:
            return Response(
                data={
                    "message": "Cover Image with id: {} does not exist".format(kwargs["pk"])
                },
                status=status.HTTP_404_NOT_FOUND
            )
    ...
    def put(self, request, *args, **kwargs):
        try:
            a_localcoverimage = self.queryset.get(pk=kwargs["pk"])
            serializer = LocalCoverImagesSerializer()
            updated_localcoverimage = serializer.update(a_localcoverimage, request.data)
            return Response(LocalCoverImagesSerializer(updated_localcoverimage).data)
        except LocalCoverImages.DoesNotExist:
            return Response(
                data={
                    "message": "No image with that id: {} exists:.".format(kwargs["pk"])
                },
                status=status.HTTP_404_NOT_FOUND
            )
...
class ProfileList(APIView):
    ...
    template_name = 'ilsapi/profile_list.html'
    permission_classes = (permissions.IsAuthenticated,)
...
class MySuggestionsListView(APIView):
    #GET suggestions/
    renderer_classes = [TemplateHTMLRenderer]
    template_name = 'ilsapi/mysuggestions.html'
    serializer_class = SuggestionsSerializer
    permission_classes = (permissions.IsAuthenticated,)

    def get(self, request):
        queryset = Suggestion.objects.filter(suggestedby__email=request.user)
        return Response({'suggestions': queryset})
