...
jQuery(function(){
    ...
    //main menu
    $("#library_banner_menu ul").singlePageNav({offset: $('#library_banner_menu').outerHeight()});
    //banner slide
    ...
    $(window).on("load scroll resize", function(){
        ...
        $("#library_banner_slide > ul > li").css({"background-size":"cover"});
    });
    //about icon
    $(window).on("load scroll resize", function(){
        ...
        $("#library_about .about_icon .imgwap i").css({
	...
	*/
    ...
    $(window).on("load resize", function(){
        ...
        $(".time_line_wap").each(function(){
            ...
            if($(window).width()<970){
                $("#library_timeline .container-fluid").css({"position":"absolute"});
                positon_left = $("#library_timeline .container-fluid").position().left +100;
                //put on right
                $(this).css({   
                                    'left': 70,
                ...
                $("#library_timeline .container-fluid").css({"position":"relative"});
                //put on left
                $(this).css({   
                                    'left':0,
                                    'top':0,
            ...
            if($.timeline_left_position_top > $.timeline_right_position_top ){
                $("#library_timeline .container-fluid").height($.timeline_left_position_top-40);
                $("#library_timeline").height($.timeline_left_position_top+200);
            }else{
                $("#library_timeline .container-fluid").height($.timeline_right_position_top-40);
                $("#library_timeline").height($.timeline_right_position_top+200);
            }
            $(this).fadeIn();
        });
    });
    //mobile menu and desktop menu
    $("#library_mobile_menu").css({"right":-1500});
    $("#mobile_menu").click(function(){
            $("#library_mobile_menu").show();
            $("#library_mobile_menu").animate({"right":0});
            return false;
    });
    $(window).on("load resize", function(){
            if($(window).width()>768){
    ...
    jQuery.fn.anchorAnimate = function(settings) {
        ...
        return this.each(function(){
            ...
            $(caller).click(function (event){
                ...
                var destination = $(elementClick).offset().top - $('#library_banner_menu').outerHeight() ;
                $("#library_mobile_menu").animate({"right":-1500});
                $("#library_mobile_menu").fadeOut() ;
    ...
    $("#library_mobile_menu a").anchorAnimate();    
    //about
    $(document).scroll(function(){
        document_top = $(document).scrollTop();
        event_wapper_top = $("#library_about").position().top - $('#library_banner_menu').outerHeight();
    ...
    $(document).scroll(function(){
        document_top = $(document).scrollTop();
        event_wapper_top = $("#library_events").position().top - $('#library_banner_menu').outerHeight();
        if(document_top<event_wapper_top){
            event_animate_num = event_wapper_top - document_top;
            event_animate_alpha = (1/event_wapper_top)*(document_top);
            $("#library_events .event_animate_left").css({'left': -event_animate_num,'opacity':event_animate_alpha});
            $("#library_events .event_animate_right").css({'left':event_animate_num,'opacity':event_animate_alpha});
        }else{
            $("#library_events .event_animate_left").css({'left': 0,'opacity':1});
            $("#library_events .event_animate_right").css({'left':0,'opacity':1});
        }
    }); 
});
//google map
function initialize(){
    ...
    map = new google.maps.Map(document.getElementById('library_contact_map'), mapOptions);
